            ...
            <div class="col-xs-5 col-sm-5 col-md-5 col-lg-10 content">
                        ...
                        <div class="col-xs-15 col-sm-15 col-md-15 col-lg-15 report">
                            <div class="report-green">
                                <div class="content-rep">
                                    <p> <i class="fa fa-signal"></i>Doanh số {{formatPrice(doanhso)}} ₫</p>
                                </div>
                            </div>
                            <div class="report-green">
                                <div class="content-rep">
                                    <p> <i class="fa fa-shopping-cart"></i>Trả hàng 0</p>
                                </div>
                            </div>
                            <div class="report-green">
                                <div class="content-rep">
                                    <p> <i class="fa fa-undo"></i>Tiền vốn {{formatPrice(sumvon)}} ₫</p>
                                </div>
                            </div>
                            <div class="report-green">
                                <div class="content-rep">
                                    <p> <i class="fa fa-cloud"></i>Lợi nhuận {{formatPrice(doanhso - sumvon)}} ₫</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-xs-15 col-sm-15 col-md-15 col-lg-15 interfact">
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                <tr v-for='(item,index) in listprofit'>
                                    <td class="text-center">{{index+1}}</td>
                                    <td class="text-center">{{item.name}}</td>
                                    <td class="text-center">Phạm Qúy</td>
                                    <td class="text-center">{{item.created_at}}</td>
                                    <td class="text-center">{{formatPrice(item.sum)}} ₫</td>
                                    <td class="text-center">{{formatPrice(summoney(item.bill_details)*sum(item.bill_details))}} ₫</td>
                                    <td class="text-center">{{formatPrice((item.sum) - summoney(item.bill_details)*sum(item.bill_details))}} ₫</td>
                                    <td class="text-center">{{formatPrice(sum(item.bill_details))}} ₫</td>
                                </tr>
                            </tbody>
                        </table>
...
import {cloneDeep} from 'lodash'
	export default{
        ...
        methods:{
            ...
            getProfit(){
                axios.get('/admin/profit/list').then(response=>{
                    console.log(response.data.profit)
                    this.listprofit = response.data.profit
