                        ...
                        <table class="table table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Tổng tiên</th>  
                                    <th class="text-center"></th>                     
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for='(item,index) in listPhieuchi'>
                                    <td class="text-center">{{index + 1}}</td>
                                    <td class="text-center">{{item.created_at}}</td>
                                    <td class="text-center">Mua hàng</td>
                                    <td class="text-center">{{formatPrice(item.sum)}} ₫</td>
                                    <td>
                                        <i class="fa fa-trash-o delete" title="Xóa" @click='deletepayment(item,index)'></i>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
        ...
        methods:{
            getreicve(){
                axios.get('/listtrade').then(response=>{
                    this.listPhieuchi = response.data.listTrade
            ...
            sum(item){
              ...
              dateSearch(){
                 axios.get('searchdatephieuchi/'+this.dateStar + '/' + this.dateEnd)
                 .then(response=>{
                    this.listPhieuchi = response.data.listPhieuchi
            ...
            deletepayment(item,index){
                axios.get('/deletepayment/'+item.id).then(response=>{
